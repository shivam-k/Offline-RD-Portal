  	...
  	if(!empty($sspi) ) {
  		if($sspi == $pi_admin) {
  	         ...
  	         $sppi = '';
    			...
    			$sql = "SELECT * FROM register WHERE pi = '$sspi' ";
    			$run_query = mysqli_query($dbcon, $sql);
    			if(mysqli_num_rows($run_query) > 0) {
              while($row = mysqli_fetch_assoc($run_query)) {
               		$name = $row['name'];
    		...
    		if(mysqli_num_rows($run_query) > 0) { // entered PI Id for delete exists in database
    			...
    			if(mysqli_num_rows($run_query) > 0) {	// entered PI Id for delete is in admin's group
                	...
                	if($pi1 == $pi_admin) { // if loggedin participants is admin or not
                     ...
                     if($ssdelete == $pi1) {
                          $sql = "UPDATE ssevent SET pi1 = '' WHERE teamid = '$senso_admin'";
                          if(mysqli_query($dbcon, $sql)) {
                            // echo "<script>alert('made space in colmn2 in event table..')</script>";    
                          }
                      }
                  		else if($ssdelete == $pi2) {
                      ...
                      $insert = "UPDATE register SET senso = '' WHERE pi = '$ssdelete'";
                      if(mysqli_query($dbcon, $insert) > 0) {
	...
	if(!empty($pppi) ) {
		if($pppi == $pi_admin) {
		   ...
		   $pppi = '';
			...
			$sql = "SELECT * FROM register WHERE pi = '$pppi' ";
			$run_query = mysqli_query($dbcon, $sql);
			if(mysqli_num_rows($run_query) > 0) {
            	while($row = mysqli_fetch_assoc($run_query)) {
             		$name = $row['name'];
              ...
              if(mysqli_num_rows($run_query) > 0) { // entered PI Id for delete is in admin's group
                  ...
                  if($pi1 == $pi_admin) { // if loggedin participants is admin or not
                      // echo "<script>alert('you are admin')</script>"; 
                       ...
                       if($ppdelete == $pi1) {
                            $sql = "UPDATE ppevent SET pi1 = '' WHERE teamid = '$paper_admin'";
                            if(mysqli_query($dbcon, $sql)) {
                                 // echo "<script>alert('made space in colmn2 in event table..')</script>";    
                            }
                        }
                       else if($ppdelete == $pi2) {
                      ...
                      $insert = "UPDATE register SET paper = '' WHERE pi = '$ppdelete'";
                      if(mysqli_query($dbcon, $insert) > 0) {
                          echo "<script>alert('Admin, $ppdelete is not anymore in your team..')</script>";
                      }
